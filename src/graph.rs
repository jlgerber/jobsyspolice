...
pub fn find<'a, I: AsRef<str>>(criteria: Vec<I>, graph: &'a JGraph) -> Result<NodePath<'a>, JSPError> {
...
fn find_recurse<'a>(
    criteria: std::slice::Iter<String>,
    graph: &'a JGraph,
    parent: NodeIndex<DefaultIx>,
    level: u8,
    indices: Rc<RefCell<Vec<NIndex>>>
    ...
    match component {
        Some(val) => {
            ...
            for n in graph.neighbors(parent) {
                let node = &graph[n];
                match *node.identity() {
                    NodeType::RegEx{name, pattern:_, exclude: _} =>  unimplemented!(),
                    NodeType::Simple(name) =>  {
                        let r = find_recurse(criteria.clone(),graph, n, level, indices.clone());
                        if r.is_success() {
                            indices.borrow_mut().push(n);
                            return ReturnValue::Success(indices);
                        }
                    },
                    NodeType::Root =>  unimplemented!(),
                    NodeType::Untracked => unimplemented!!("replace this "),
                }
                trace!("testing {:?} against {:?}", val, node);
                if node == val {
                    trace!("MATCH");
